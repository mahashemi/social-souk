...
// Split stored "+92 3001234567" into dial code + number for the form
$dialCode = '';
$phoneNumber = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        if ($newPass !== '') {
            ...
            $pdo->prepare('UPDATE users SET name=?, city=?, country=?, bio=?, phone=?, password=? WHERE id=?')
                ->execute([$name, $city, $country, $bio, $phone, $hash, $user['id']]);
        } else {
            $pdo->prepare('UPDATE users SET name=?, city=?, country=?, bio=?, phone=? WHERE id=?')
                ->execute([$name, $city, $country, $bio, $phone, $user['id']]);
        }

        $_SESSION['user']['name'] = $name;
        $success = true;
        $me['name'] = $name; $me['city'] = $city; $me['country'] = $country; $me['bio'] = $bio;
    ...
    <div class="card"><div class="card-body">
        <form method="post">
            ...
            <div class="form-group">
                ...
                <textarea name="bio" class="form-control" placeholder="Tell buyers a little about yourself..."><?= e($me['bio'] ?? '') ?></textarea>
            </div>
